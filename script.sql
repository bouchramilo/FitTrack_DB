...
-- exercice 7 : refaire
SELECT * FROM trainers WHERE (specialization = 'Musculation Coach' or specialization = 'Cardio Specialist');
-- ou bien 
SELECT * FROM trainers WHERE specialization IN ('Musculation Coach', 'Cardio Specialist');
-- ou bien 
SELECT * FROM trainers WHERE id_department IN (SELECT * FROM departments WHERE department_name ='Musculation' or department_name = 'Cardio');
...
-- exercice 11 :
SELECT id_department AS 'Departement_ID', count(*) As 'Nombre d entraineur' FROM trainers GROUP BY id_department ;

-- exercice 12 :
SELECT AVG(YEAR(CURDATE()) - YEAR(date_of_birth)) AS avg_age FROM members WHERE date_of_birth IS NOT NULL;

-- exercice 13 :
SELECT MAX(CONCAT(appointment_date, ' ', appointment_time)) AS last_appointment FROM appointments;

-- exercice 14 :
SELECT SUM() FROM memberships GROUP BY id_room ;

-- 
SELECT id_room, COUNT(*) AS total_abonnements FROM memberships GROUP BY id_room;
...
-- exercice 19 :
SELECT gender, COUNT(*) AS total_members FROM members GROUP BY gender HAVING COUNT(*) >= 2;
-- ou bien 
SELECT gender, COUNT(*) AS total_members FROM members GROUP BY gender HAVING total_members >= 2;
